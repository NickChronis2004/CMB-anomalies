...
\begin{enumerate}
  ...
  \item The Savage--Dickey Bayes factor satisfies $|\ln B_{01}| < 1$,
    indicating no meaningful statistical preference for either model. The
...
for a range of $\ln D$ values at fixed $\alpha = 2$, which represents a
moderate transition shape; sharper profiles ($\alpha > 2$) would yield
higher SNR at fixed $k_c$. For the Contaldi et al.\
...
most promising near-term avenue for improving constraints: its low-noise
polarisation measurements effectively provide an independent realisation
of the low-$\ell$ sky, reducing the cosmic variance floor by
approximately $\sqrt{2}$ and lifting the Contaldi-like case from
$1.7\sigma$ to $2.4\sigma$. A definitive ($\geq 3\sigma$) detection
requires the suppression to be stronger than what is indicated by the
Planck best-fit. The complementary constraint from the tensor-to-scalar
ratio (section~\ref{subsec:tensor_ratio}) and future large-scale
structure surveys probing $k \lesssim 10^{-3}\,\mathrm{Mpc}^{-1}$ may
provide additional sensitivity beyond what is achievable with CMB data
alone.
...
\paragraph{Single MCMC chain.} The posterior was sampled with a single 
Metropolis--Hastings chain achieving $R-1 < 0.005$ via the split-chain 
method, well below the conventional $R-1 < 0.01$ threshold. The trace plots
and marginal distributions confirm adequate mixing (appendix~\ref{app:convergence}). 
While a single chain with split-chain diagnostics is standard practice in 
cosmological parameter estimation, future analyses could employ multiple 
independent chains and/or nested sampling to provide direct evidence computation.
...
The best-fit CCR model achieves $\chi^2_{\min} = 1002.6$ for
